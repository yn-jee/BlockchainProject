...
class myBC(fn.full_node, un.user_node):
    def __init__(self, full_num, user_num, full_links_num, genesis):    # number of full/user nodes, number of full-full links
        ...
        if __name__ == "__main__":
            ...
            # Use Bitcoin's elliptic curve and hash function to sign

            # sk, vk 쌍을 일괄적으로 생성한 뒤 user node 생성 시에 인자로 넘겨줌
            # 다른 user node의 vk 정보도 전달받음 ---- user_num - 1 길이의 일차원 배열 other_users로.
            # 프로세스 간에 키를 전달하기 위해 string 형태로 변환한 뒤 전달
            # 각 객체 생성 시 string 형태를 다시 키로 변환하여 사용

            asymm_keys = [[0, 0] for _ in range(user_num)]
            for i in range(user_num):
                asymm_keys[i][0] = ec.SigningKey.generate(curve=ec.SECP256k1, hashfunc=sha256)
                asymm_keys[i][1] = asymm_keys[i][0].verifying_key

                asymm_keys[i][0] = asymm_keys[i][0].to_string()
                asymm_keys[i][1] = asymm_keys[i][1].to_string()


            for i in range(user_num):
                sk = asymm_keys[i][0]
                vk = asymm_keys[i][1]
                other_users = [asymm_keys[j][1] for j in range(user_num) if j != i]

                temp_user = un.user_node(i, queue_user[i], sk, vk, stack_matrix_user, other_users)
